...
class ShurjopayPdfController extends Controller
{
    ...
    public function pdfBookCheckout(Request $request)
    {
        ...
        $order_num = 'SPDF-' . strtoupper(substr(md5(uniqid()), 0, 6)) . '-' . date('Ymd');

        session()->put('pending_pdf_book_order_shurjopay', [
            'user_id' => Auth::id(),
            'pdf_book_id' => $book->id,
            'name' => $request->name,
            'email' => $request->email,
        ...
        $payload = [
            ...
            "token" => $token,
            "return_url" => route('pdf.book.shurjopay.success'),
            "cancel_url" => route('pdf.book.shurjopay.cancel'),
        ...
        session()->forget('pending_pdf_book_order_shurjopay');
        return redirect()->back()->with('error', 'Shurjopay payment creation failed.');
    }

    public function pdfBookSuccess(Request $request)
    {
        $orderId = $request->query('order_id');
        $config = ShurjopaySetting::first();
        $token = $this->getToken($config);
        $pendingData = session()->get('pending_pdf_book_order_shurjopay');

        if (!$orderId) {
            return redirect()->route('pdf.book.payment.fail')->with('error', 'Missing order_id from Shurjopay.');
        }
        ...
        session()->forget('pending_pdf_book_order_shurjopay');
        return redirect()->route('pdf.book.payment.fail')->with('error', 'Shurjopay verification failed.');
    }

    public function pdfBookCancel()
    {
        session()->forget('pending_pdf_book_order_shurjopay');
