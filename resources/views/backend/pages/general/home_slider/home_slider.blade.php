                        ...
                        <div class="form-row">
                            ...
                            <div class="col-12 mb-3">
                                <div class="form-group">
                                    ...
                                    <input type="file" name="slider_image" class="dropify" data-default-file=""
                        ...
                        <table id="file-datatable" class="table table-bordered text-nowrap key-buttons border-bottom w-100">
                            ...
                            <tbody>
                                @foreach ($home_sliders as $home_slider)
                                    <tr>
                                        <td>
                                            <strong>{{ $home_sliders->firstItem() + $loop->index }}</strong>
                                        </td>
                                        <td>{{ $home_slider->updated_at->format('d-M-Y') }}</td>
                                        <td>{{ $home_slider->title }}</td>
                                        <td>{{ $home_slider->description }}</td>
                                        <td>
                                            <img src="{{ asset('uploads/home_slider') }}/{{ $home_slider->slider_image }}" alt="" style="height: 100px">
                                        </td>
                                        {{-- @can('edit-permission') --}}
                                        <td class="text-center">
                                            <div class="action-btns d-flex align-items-center">
                                                ...
                                                {{-- @can('edit-permission') --}}
                                                <div>
                                                    <a href="{{ route('home_slider.edit', $home_slider->id) }}"
                                                ...
                                                <div>
                                                    <form action="{{ route('home_slider.destroy', $home_slider->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
