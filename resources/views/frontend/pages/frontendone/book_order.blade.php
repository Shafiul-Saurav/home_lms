...
@push('frontend_style')
    <style>
        ...
        @media (min-width: 768px) {
            .user-table .order-responsive-table tbody tr td:nth-child(2) {
...
@section('frontend_content')
    <main class="main">
        <!-- breadcrumb -->
        <x-frontend.pages.common.breadcrumb :title="'Book Orders'" :breadcrumb="[['name' => 'Home', 'url' => route('home')], ['name' => 'Book Orders', 'url' => '#']]" />
        <!-- breadcrumb end -->
            ...
            <div class="container">
                <div class="row g-4">
                    <div class="col-lg-4 col-xl-3">
                        @include('frontend.pages.frontendone.sidebarmenu.sidebar')
                    </div>
                    <div class="col-lg-8 col-xl-9">
                        <div class="user-wrapper">
                            <div class="user-card">
                                <div class="header">
                                    <h4 class="title">Book Orders</h4>
                                    <div class="right">
                                        <div class="filter">
                                            <select class="select">
                                ...
                                <div class="user-table table-responsive">
                                    <table class="table table-borderless text-nowrap order-responsive-table">
                                        ...
                                        <tbody>
                                            @forelse($orders as $order)
                                                ...
                                                <tr>
                                                    <td class="responsive-toggle-col">
                                                        <button class="responsive-row-toggle" type="button" aria-expanded="false" aria-label="Show order details">
                                                            <i class="far fa-plus"></i>
                                                            <i class="far fa-minus"></i>
                                                        </button>
                                                    </td>
                                                    <td><span class="code">{{ $order->order_number ?? sprintf('#%s', str_pad($order->id, 6, '0', STR_PAD_LEFT)) }}</span></td>
                                                    <td class="responsive-hidden">{{ $order->created_at?->format('F j, Y') ?? optional($order->date)->format('F j, Y') }}</td>
                                                    <td class="responsive-hidden">{{ $order->currency ?? 'BDT' }}{{ number_format($order->amount, 2) }}</td>
                                                    <td class="responsive-hidden"><span class="badge {{ $badgeClass }}">{{ $displayStatus }}</span></td>
                                                    <td>
                                                        <div class="action-dropdown dropdown">
                                                            ...
                                                            <ul class="dropdown-menu dropdown-menu-end" style="max-height: none !important; overflow: visible !important;">
                                                                <li>
                                                                    <a class="dropdown-item" href="{{ route('book.order.details', $order) }}"><i class="far fa-file-alt"></i> Order Details</a>
                                                                </li>
                                                                <li>
                                                                    <a class="dropdown-item" href="{{ route('book.details', $order->book_id) }}"><i class="far fa-eye"></i> View Book</a>
                                                                </li>
                                                            </ul>
                                                        </div>
                                                    </td>
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="6" class="text-center">You have no book orders yet.</td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                    </table>
            ...
            document.querySelectorAll('.order-responsive-table').forEach(function (table) {
                table.querySelectorAll('.responsive-row-toggle').forEach(function (button) {
                    button.addEventListener('click', function () {
                        ...
                        if (!childRow || !childRow.classList.contains('responsive-child-row')) {
                            childRow = document.createElement('tr');
                            childRow.className = 'responsive-child-row';
                            childRow.innerHTML = '<td colspan="' + row.children.length + '"><div class="responsive-child-list"></div></td>';

                            row.querySelectorAll('td.responsive-hidden').forEach(function (cell) {
                                var columnIndex = Array.prototype.indexOf.call(row.children, cell);
                                var label = table.querySelectorAll('thead th')[columnIndex].textContent.trim();
                                var item = document.createElement('div');
                                item.className = 'responsive-child-item';
                                item.innerHTML = '<span class="responsive-child-label">' + label + '</span><span class="responsive-child-value">' + cell.innerHTML + '</span>';
                                childRow.querySelector('.responsive-child-list').appendChild(item);
                            });
