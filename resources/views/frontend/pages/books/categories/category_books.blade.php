...
@section('frontend_content')
    <main class="main">
        <!-- breadcrumb -->
        <x-frontend.pages.common.breadcrumb :title="$category->name . ' Books'" :breadcrumb="[['name' => 'Home', 'url' => route('home')], ['name' => 'Books', 'url' => route('books')], ['name' => $category->name, 'url' => '#']]" />
        <!-- breadcrumb end -->
                                    ...
                                    <ul>
                                        <li>
                                            <div class="form-check">
                                                <input class="form-check-input category-filter" type="checkbox" name="category" value="" id="cat0" {{ empty(request('category')) && !isset($category) ? 'checked' : '' }} />
                                        ...
                                        <li>
                                            <div class="form-check">
                                                <input class="form-check-input category-filter" type="checkbox" name="category" value="{{ $cat->id }}" id="cat{{ $cat->id }}" {{ (request('category') ? in_array($cat->id, explode(',', request('category'))) : (isset($category) && $category->id == $cat->id)) ? 'checked' : '' }} />
...
<script>
    $(document).ready(function() {
        ...
        function buildFilterUrl(pageUrl = null) {
            ...
            return "{{ route('books') }}?" + urlParams.toString();
        }
