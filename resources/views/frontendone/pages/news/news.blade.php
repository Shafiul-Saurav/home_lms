...
@section('frontendone_content')
    <main class="main" data-aos="fade-up">
        <!-- breadcrumb -->
        <x-frontend.pages.common.breadcrumb :title="'News'" :breadcrumb="[['name' => 'Home', 'url' => route('home')], ['name' => 'News', 'url' => '#']]" />
        <!-- breadcrumb end -->
        ...
        <div class="blog-area py-5">
            <div class="container">
                ...
                {{-- <div class="row mb-4">
                    <div class="col-lg-6 mx-auto text-center">
                        <div class="site-heading wow fadeInDown" data-wow-delay=".25s">
                            <span class="site-title-tagline"><i class="far fa-lightbulb-on"></i> Latest Updates</span>
                            <h2 class="site-title">Our Latest <span class="text-gradient">News</span></h2>
                        </div>
                    </div>
                </div> --}}
                ...
                <div class="text-center mt-4 mb-5">
                    @auth
                        <button type="button" class="nav-action mt-5" style="border: none;" onclick="openCpModal()">Submit
                            News</button>
                    @else
                        <a href="{{ route('login') }}" class="nav-action mt-5">Login to Submit News</a>
                    @endauth
                </div>
        ...
        {{-- ════════════════════════════════════════════
             CREATE NEWS MODAL (auth users only)
        ═══════════════════════════════════════════════ --}}
        @auth
            <div class="cp-overlay" id="cpOverlay" onclick="closeCpModalOutside(event)">
                <div class="cp-modal" id="cpModal">
                    ...
                    <div class="cp-modal-header">
                        <div>
                            <div class="badge-tag"><i class="fa-solid fa-pen-to-square"></i> Submit News</div>
                            <h4>Create News Article</h4>
                            <p>Share breaking news or important updates with our community.</p>
                        </div>
                    ...
                    <div class="cp-modal-body">
                        ...
                        <div class="notice-box warning">
                            ...
                            <span>Your news submission will be <strong>reviewed by admin</strong> before it goes live. This
                                usually takes 1–2 business days.</span>
                        </div>
                        ...
                        <form action="{{ route('frontend.news.store') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            {{-- Title --}}
                            <div class="cp-form-group">
                                <label for="cp_title">News Title <span class="req">*</span></label>
                                <input type="text" name="title" id="cp_title"
                                    class="cp-input @error('title') is-invalid @enderror"
                                    placeholder="Enter a compelling headline…" value="{{ old('title') }}" maxlength="255">
                                @error('title')
                                    <div class="cp-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="cp-form-group">
                                ...
                                <textarea name="short_des" id="cp_short_des" rows="2" class="cp-textarea @error('short_des') is-invalid @enderror"
                                    placeholder="1–2 sentence summary…">{{ old('short_des') }}</textarea>
                                @error('short_des')
                                    <div class="cp-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="cp-row">
                                <div class="cp-form-group">
                                    ...
                                    <textarea name="description" id="cp_description" rows="4"
                                        class="cp-textarea @error('description') is-invalid @enderror" placeholder="Main news content…">{{ old('description') }}</textarea>
                                    @error('description')
                                        <div class="cp-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}
                                        </div>
                                    @enderror
                                </div>
                                <div class="cp-form-group">
                                    ...
                                    <textarea name="long_des" id="cp_long_des" rows="4"
                                        class="cp-textarea @error('long_des') is-invalid @enderror" placeholder="Full detail, context and depth…">{{ old('long_des') }}</textarea>
                                    @error('long_des')
                                        <div class="cp-error"><i class="fa-solid fa-circle-exclamation"></i> {{ $message }}
                                        </div>
                                    @enderror
                            ...
                            {{-- Cover Image --}}
                            <div class="cp-form-group">
                                <label for="cp_news_image">
                                    Cover Image
                                    <span style="color:#6b7280;font-weight:500;text-transform:none;"> (optional · jpg/png, max
                                        2 MB)</span>
                                </label>
                                <div class="cp-upload-zone">
                                    <input type="file" name="news_image" id="cp_news_image" accept=".jpg,.jpeg,.png"
                            ...
                            <button type="submit" class="cp-submit-btn">
                                <i class="fa-solid fa-paper-plane"></i> Submit News for Review
                            </button>
                        </form>
                    </div>{{-- /.cp-modal-body --}}
                </div>{{-- /.cp-modal --}}
